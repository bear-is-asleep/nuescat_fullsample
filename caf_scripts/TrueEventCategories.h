...
const SpillVar kBestNuID([](const caf::SRSpillProxy* sp) {
    ...
    for(auto const& nu : sp->mc.nu)
      {
        //if (nu.genie_inttype == 1098){print_all_prim_info_nu(sp,counter);return counter;} //Best possible truth match
    ...
    //if (sp->mc.nu[id].genie_inttype == 1098) print_all_prim_info_nu(sp,id);
    return id;
  });
