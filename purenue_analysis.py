...
NuEScat_dir = f'/sbnd/data/users/brindenc/analyze_sbnd/nue/states/2022A/2023_2_23_pure_nue_truth_cuts'
surname = ''
...
def plot_all_keys(df,ftype='.png'):
  ...
  for key_x in df.keys():
    for key_y in df.keys():
      ...
      fnames.append(f'dist_{key_x.replace(".","_")}_{key_y.replace(".","_")}')
...
other_events = helpers.remove_dummy_values(other_events,dummy_val_list=[-9999,-999])

#Get reco and true labels and arrays
reco_labels_other = ['nshw','ntrk','reco_eng','reco_theta',
                     'reco_vtx.x','reco_vtx.y','reco_vtx.z','Etheta']
...
true_labels_other = ['truenshw','truentrk','true_slice_eng','true_theta',
                     'true_vtx.x','true_vtx.y','true_vtx.z']
...
etheta_bins = np.arange(0,0.05,0.0005)
