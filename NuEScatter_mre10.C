/*
MRE10 is the minimum reproducible example when running the original cuts on the MCP2022A sample. v09_37
The numbers between MRE1,MRE2,MRE3,MRE10 should be the exact same for their respective samples within their
statistical uncertainty. They're all normalized to the same POT = 10e20. This selection looks at all 
true categories. We will look at a few energy distributions just to speed up and get the event rates.
*/
...
const std::vector<Plot<SpillVar>> plots = recoPlots_eng;
const std::vector<TrueCategory> categories = nuecc_nue_sel;
const string surName = "mre10";

void NuEScatter_mre10(bool reload = true)
{
...
gSystem->Exec("cp NuEScatterCuts.h NuEScatterRecoVars.h TrueEventCategories.h NuEScatter_mre10.C " + saveDir);
}